...
export const iniciarAdopcion = async (req, res) => {
    try {
        ...
        if (mascota.length > 0) {
            ...
            const [result] = await pool.query("UPDATE mascotas SET estado = 'proceso adopcion', fk_id_usuario = ? WHERE id_mascota = ?", [id_usuario, id_mascota]);
...
export const administrarAdopcion = async (req, res) => {
    try {
        ...
        const [mascotaResult] = await pool.query("SELECT * FROM mascotas WHERE id_mascota = ?", [id_mascota]);
        ...
        const [usuarioResult] = await pool.query("SELECT * FROM usuarios WHERE id_usuario = ?", [mascota.fk_id_usuario]);
        ...
        if (accion === 'aceptar') {
            const [updateResult] = await pool.query("UPDATE mascotas SET estado = 'adoptada' WHERE id_mascota = ?", [id_mascota]);
            ...
            const [updateResult] = await pool.query("UPDATE mascotas SET estado = 'adoptar' WHERE id_mascota = ?", [id_mascota]);
            if (updateResult.affectedRows > 0) {
                res.status(200).json({
                    status: 200,
    ...
    try {
        const [mascotasResult] = await pool.query(
            `SELECT m.*, u.id_usuario, u.nombres, u.apellidos, u.correo, u.numero_cel
             ...
             WHERE m.estado = 'proceso adopcion'`
        );
...
export const registrarMascota = async (req, res) => {
	try {
		const { nombre, genero, raza, edad, descripcion, estado, fk_id_usuario, especie, esterilizacion } =
			req.body;
		...
		const [result] = await pool.query(
			...
			[nombre, genero, raza, edad, img, descripcion, estado, fk_id_usuario, especie, esterilizacion]
		);
